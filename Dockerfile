...
RUN apk add --update wget ca-certificates curl && \
  ...
  wget -qO- -O nomad.zip https://dl.bintray.com/mitchellh/nomad/nomad_${NOMAD_VERSION}_linux_amd64.zip --no-check-certificate && \
  unzip -d /usr/bin/ nomad.zip && rm nomad.zip && chmod +x /usr/bin/nomad && \
  apk del wget && \
  rm -rf /var/cache/apk/*
